...
kubectl get cronjobs -n keycloak -o name | while read -r cj; do
  ...
  if echo "$CJ_SPEC" | grep -qi "reconcil\|group\|membership\|glitchtip\|backup-sync\|owners"; then
...
kubectl get jobs -n keycloak -o name 2>/dev/null | while read -r job; do
  ...
  if echo "$JOB_SPEC" | grep -qi "reconcil\|realm-config\|backup-sync\|realm-backup"; then
